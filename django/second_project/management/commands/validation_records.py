...
def _last_completed_at(state_path: Path) -> datetime | None:
    ...
    value = payload.get("last_completed_at")
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser) -> None:
        ...
        parser.add_argument(
            "--input-jsonl",
            type=Path,
            default=django_root / "data" / "raw_data" / "records.jsonl",
            help=(
                "입력 JSONL 경로 "
                f"(기본값: {django_root / 'data' / 'raw_data' / 'records.jsonl'})"
            ),
        )
        parser.add_argument(
            "--rules",
        ...
        parser.add_argument(
            "--log-directory",
            type=Path,
            default=django_root / "log_rake",
            help=(
                "표준화·검증 로그 디렉터리 "
                f"(기본값: {django_root / 'log_rake'})"
            ),
        )
        parser.add_argument(
            "--database-alias",
    ...
    def _run_if_due(self, options) -> int:
        """Run only when the optional minimum interval permits a new load."""
        ...
        exit_code = self._run_pipeline(options)
        if exit_code == 0 and interval_hours > 0:
            try:
    ...
    def _run_pipeline(self, options) -> int:
        ...
        input_path = Path(options["input_jsonl"]).expanduser().resolve()
        ...
        self.stdout.write(
            "JSONL validation pipeline을 실행합니다. "
            f"input={input_path} rules={rules_path} "
        ...
        try:
            config = AppConfig(
                ...
                sink=SinkConfig(
                    ...
                    bronze_database=options["success_database"],
