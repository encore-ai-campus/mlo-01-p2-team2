...
6. Gold 계층은 추천 피처 생성, 후보 점수 산정, Top-K 추천, 서빙 구조 및 Django 화면구현

### Gold 대체인력 후보 추천·검토 MVP

후속 승인에 따라 Gold 추천 결과를 제공하는 Django 내부 검토 화면을 구현했다.

후보별 추천 점수, Top-K 후보 목록, 주요 추천 근거와 데이터 품질 상태를 조회할 수 있으며, 최종 인사 결정은 담당자가 수행한다.
