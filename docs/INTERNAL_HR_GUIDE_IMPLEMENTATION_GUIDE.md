...
이 순서는 인사결정 우선순위나 추천점수가 아니다. HR 화면에도 우선대상을 뜻하지 않는다는 안내가 표시된다.
...
| `ON_HOLD` | 데이터 확인 전 판단 불가 | 관계 누락, 프로필 누락 또는 충돌 때문에 확정 불가 | 데이터 정정 전 내부대체·채용 판단 보류 |
...
| `docs/INTERNAL_HR_GUIDE_EXECUTION_PLAN.md` | 승인 Gate, 통합 태스크, 상태, KPI 및 정책 결정 |
| `docs/INTERNAL_HR_GUIDE_UAT.md` | HR·팀 관리자·데이터·보안 UAT 체크리스트 |
| `django/HR_GUIDE_RUNBOOK.md` | 설치, 환경변수, migration, 권한, 실행, Smoke Test 및 롤백 절차 |
| `docs/INTERNAL_HR_GUIDE_IMPLEMENTATION_GUIDE.md` | 현재 구현물 전체를 연결한 본 통합 설명서 |
| `docs/INTERNAL_HR_GUIDE_FILE_HANDOFF.md` | 파일별 책임·의존성·테스트·변경 주의사항 인수인계 |
...
| `EMP900030` | 데이터 확인 전 판단 불가 | 대상 현재값에 `DATE_CONFLICT`가 있는 경우 |
...
2. `docs/INTERNAL_HR_GUIDE_IMPLEMENTATION_GUIDE.md` — 전체 구현 구조
3. `docs/INTERNAL_HR_GUIDE_EXECUTION_PLAN.md` — 승인 Gate와 태스크 현황
4. `docs/INTERNAL_HR_GUIDE_FILE_HANDOFF.md` — 담당 파일별 구현 인계
5. `docs/INTERNAL_HR_GUIDE_UAT.md` — 실사용 승인 시나리오
