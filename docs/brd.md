...
| 규칙 기반 표시 순서가 최종 인사 우선순위로 오인될 가능성 | 자동 결정으로 오해 | 화면에 숫자 점수를 표시하지 않고 최종 결정은 담당자가 수행한다고 안내 | mitigated |
...
| 후보 검토 결과 확인 가능 | 부서·직위·근속기간 기준으로 정렬된 후보 목록과 주요 검토 근거를 화면에서 확인할 수 있으며, 숫자 후보 점수와 자동 선정은 제공하지 않음 |
