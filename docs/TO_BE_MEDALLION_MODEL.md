...
| Gold | 확정·구현 완료 | Gold 데이터 패키지, 규칙 기반 후보 정렬 및 Django 내부 검토 화면 구현 |
...
8. Silver 품질 게이트를 통과한 데이터를 결합해 Gold AI Ready 데이터셋과 배포 패키지를 생성한다.
9. 재직 상태와 데이터 품질 조건을 통과한 후보를 부서·직위·근속기간 기준으로 정렬한다.
10. 정렬된 후보와 검토 근거를 Django 내부 검토 화면으로 제공한다.
...
| 후보 정렬 | 부서 일치·직위 일치·근속기간·이름·직원 ID 순으로 결정적 정렬 |
| 결과 재현 | 동일 입력과 기준시점에서 동일 후보 순서 재현 |
...
대체인력 후보를 조회하고 정렬하기 위해 다음 정보를 활용한다.
...
후보의 재직 상태와 근속기간은 실행 기준시점인 `as_of_date`를 기준으로 판단한다.

### 8.3 규칙 기반 후보 정렬

1. 퇴직 관리자가 담당하던 업무영역을 확인한다.
2. Silver 데이터에서 추천 대상이 될 수 있는 후보 관리자를 조회한다.
3. 재직 상태와 데이터 품질 차단 조건을 적용한다.
4. 부서 일치 여부, 직위 일치 여부, 근속기간 순으로 후보를 정렬한다.
5. 동일 조건에서는 이름과 직원 ID를 사용해 결과 순서를 결정한다.
6. 정렬된 후보 목록과 주요 검토 근거를 화면에 제공한다.

숫자 후보 점수와 최소점수는 사용하지 않는다. 동일한 데이터, 규칙 및 `as_of_date`로 실행하면 동일한 후보 순서가 생성되어야 한다.
...
Gold 후보 검토 결과는 Django 내부 검토 화면에서 제공한다.
...
- 후보의 재직 상태와 근속기간은 `as_of_date`를 기준으로 판단한다.
