...
| 김건우 | pm, 골드티어 | 화면 MVP 및 상세 구현, 시연 자료 연계 |
...
- 표준 Silver 데이터로 변환한 뒤 Gold 데이터 패키지와 규칙 기반 후보 검토 결과를 제공
- Gold 데이터 패키지, 후보 정렬 규칙 및 Django 내부 검토 화면 구현

## 2. 사용자 필요

| ID | 사용자 필요 |
|---|---|
| `UN-HR-001` | 인사 담당자는 대체인력 후보 검토에 사용할 직원·업무영역 표준 데이터와 규칙 기반 후보 목록을 확인 |
| `UN-OPS-001` | 운영 담당자는 원천부터 Silver까지 실패 위치와 재처리 대상을 확인 |
| `UN-QA-001` | 품질·검토 담당자는 RAW_DB 복원율, 원본 무결성, 레코드 계보, 변경 이유와 격리 사유를 확인 |
...
| `FR-GOLD-001` | Must | Gold 데이터 패키지, 규칙 기반 후보 정렬 및 Django 내부 검토 화면을 구현한다. 숫자 후보 점수와 자동 선정은 제공하지 않는다. | implemented |
...
| `AC-GOLD-001` | Gold 데이터 패키지·후보 정렬 규칙·화면 결과 검증 | Gold 구현 완료 | 후보 검토 실행 | 동일 입력과 기준시점에 동일한 후보 순서와 검토 근거가 제공되고 숫자 후보 점수나 자동 선정은 노출되지 않음 | Gold 결과, Django 테스트, BRD·PRD·설계 문서 |
...
Gold 구현 완료 이후 다음 항목을 고도화한다.

- 퇴직 이벤트의 정확한 업무 정의
- 과거 선정 사례를 활용한 추천 품질 평가
- 현업 평가 및 피드백 반영 방법
- 개인정보 노출·승인·감사 정책
- 숫자 후보 점수·가중치·최소점수 도입 여부 검토
