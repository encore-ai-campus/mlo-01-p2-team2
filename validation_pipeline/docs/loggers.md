...
모든 감사 로그는 UTF-8 JSON Lines로 남긴다. 기본 경로는 실행 방식에 따라 설정값을 사용하며, Windows Django 환경의 표준화·검증 로그 기준 경로는 프로젝트의 `django/log_lake/standardized`다.
...
8개 누적 로그는 Windows 내장 Python 로거로 공통 회전한다. 크롤링·Bronze 로그는
`log_lake/raw_data`, 표준화·검증 로그는 `log_lake/standardized`에 저장한다.
KST 기준 00:00, 06:00,
